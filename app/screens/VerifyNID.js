...
function VerifyNID(props) {
  ...
  const [nationalIdNum, setNationalIdNum] = useState(user.nationalIdNum);
  ...
  const handleUpdateProfileMedia = async (data) => {
    ...
    const verifyResult = await apiClientNotCached.post(
      "https://vapi.verifyme.ng/v1/verifications/identities/nin/10000000001",
      {
        firstname: "John",
        lastname: "Doe",
        phone: "080000000000",
    ...
    if (verifyResult.data) {
      await authApi.updateProfile({
        nationalIdVerificationStatus: "success",
        nationalIdNum,
      });
  ...
  return (
    <View style={styles.container}>
      <AppTextInput
        ...
        returnKeyType='done'
        defaultValue={user.nationalIdNum}
        onChangeText={(text) => setNationalIdNum(text)}
      />
      <AppTextInput
